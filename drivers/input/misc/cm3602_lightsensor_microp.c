...
#define DLS(x...) printk(KERN_DEBUG "[LS][CM3602] " x)
#define ILS(x...) printk(KERN_INFO "[LS][CM3602] " x)
#define ELS(x...) printk(KERN_ERR "[LS][CM3602 ERROR] " x)
...
static int upload_ls_table(struct microp_ls_info *li)
{
	...
	for (i = 0; i < 10; i++) {
		...
		ILS("ls_table: data[%d] , data[%d] = %x, %x\n",
				   ...
				   const char *buf, size_t count)
{
	...
	if (kadc_temp <= 0 || li->ls_config->golden_adc <= 0) {
		ILS("%s: kadc_temp=0x%x, als_gadc=0x%x\n",
			__func__,
			kadc_temp,
...
static int lightsensor_probe(struct platform_device *pdev)
{
	...
	if (!li->client) {
		ELS("%s: can't get microp i2c client\n", __func__);
		return -1;
	}
	li->ls_input_dev = input_allocate_device();
	if (!li->ls_input_dev) {
		ELS("%s: could not allocate input device\n", __func__);
		return -ENOMEM;
	}
	...
	if (ret < 0) {
		ELS("%s: can not register input device\n",
				__func__);
		return ret;
	}
	...
	if (ret < 0) {
		ELS("%s: can not register misc device\n",
				__func__);
		return ret;
	}
	...
	if (ret < 0) {
		ELS("%s: request_irq(%d) failed for (%d)\n",
			__func__, irq, ret);
		return ret;
	}
	...
	if (li->ls_config->ls_gpio_on) {
		ret = gpio_request(li->ls_config->ls_gpio_on,
				"microp_i2c");
		if (ret < 0) {
			ELS("request gpio ls failed\n");
			return ret;
		}
		ret = gpio_direction_output(li->ls_config->ls_gpio_on, 0);
		if (ret < 0) {
			ELS("gpio_direction_output ls failed\n");
			return ret;
		}
	}
	ls_power(1);
